...
Compute the effect sizes for the two dilution effects (one vs. three, one vs. seven)
...
# get SE of d 
half_CI_width <- (1.768370 - 1.161144)*0.5
SE <- half_CI_width/1.96
SE
...
write.table(tdata_17, file = "tdata_17.txt", row.names=FALSE, sep="\t", quote=FALSE, append = FALSE)
tdata_17 <- read.delim("tdata_17.txt", header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)

cohen.d(tdata_17$value, tdata_17$variable, paired = T)
d <- 1.843288

# get SE of d 
half_CI_width <- (2.259431 - 1.427145)*0.5
SE <- half_CI_width/1.96
SE
