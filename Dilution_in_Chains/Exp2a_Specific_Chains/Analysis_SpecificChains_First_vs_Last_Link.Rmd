---
title: "chain_dilution: first vs. last link"
...
tdata_sub %>% cohens_d(value ~ link, paired = TRUE)
...
# turn tdata_sub into wide format
library(tidyr)
data_wide <- spread(clean, link, value)

# 1) compute correlations for the ratings


cor.test(data_wide$first, data_wide$last)
...
```{r}
# count N in each scenario 

table(data_wide$scenario)
```
...
apples_dat %>% cohens_d(value ~ link, paired = TRUE)
...
# turn tdata_sub into wide format
library(tidyr)
apples_dat <- subset(apples_dat, select = c(1:7))
apples_wide <- spread(apples_dat, link, value)

# 1) compute correlations for the ratings


cor.test(apples_wide$first, apples_wide$last)
...
chem_dat %>% cohens_d(value ~ link, paired = TRUE)
...
# turn tdata_sub into wide format
library(tidyr)
chem_dat <- subset(chem_dat, select = c(1:7))
chem_dat_wide <- spread(chem_dat, link, value)

# 1) compute correlations for the ratings


cor.test(chem_dat_wide$first, chem_dat_wide$last)
...
comp_dat %>% cohens_d(value ~ link, paired = TRUE)
...
# turn tdata_sub into wide format
library(tidyr)
comp_dat <- subset(comp_dat, select = c(1:7))
comp_dat_wide <- spread(comp_dat, link, value)

# 1) compute correlations for the ratings


cor.test(comp_dat_wide$first, comp_dat_wide$last)
