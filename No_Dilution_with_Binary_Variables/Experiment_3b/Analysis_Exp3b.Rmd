---
title: "No dilution of causal strength with truely binary variables (Probabilistic causes)"
...
```{r}
tdata <- read.delim("data_Exp3b.txt", header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
```
...
crit = 3 # define a threshold that needs to be exceeded to leave the "invariance" cluster
...
prop.test(43,216,correct=FALSE)

prop.test(151,216,correct=FALSE)


prop.test(22,216,correct=FALSE)
...
g <- ggplot(tdata_sub, aes(x=variable, y=valueJitter, group = sID, color = Change)) +
  ...
  scale_color_manual(name = "Model",values=palette, labels = c("Dilution: 20%", "Invariance: 70%", "Strengthening: 10%"))+
