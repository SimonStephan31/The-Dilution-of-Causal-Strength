...
tdata$Capacity_Info <- factor(tdata$Capacity_Info, levels = c("none", "same", "different"))
...
tdata_sub <- tdata_sub %>% gather(variable, value, 5:7)
...
g <- ggplot(tdata_sub, aes(x=variable, y=valueJitter, group = sID)) +
  guides(fill=FALSE)+
  facet_grid( ~ Capacity_Info, labeller = labeller(Capacity_Info = capacity.labs))+
...
by(tdata_sub$value, list(tdata_sub$variable, tdata_sub$Capacity_Info), stat.desc , basic = FALSE)
...
a1 <- aov_car(value ~ Capacity_Info*Crystal_Order*Q_Order + Error(sID/(variable)), tdata_sub)
a1
...
lmeModel <- lmer(value ~ Capacity_Info*Crystal_Order*Q_Order*variable + (1|sID), data=tdata_sub)

# follow-up analysis 

ls1 <- lsmeans(a1, c("variable", "Capacity_Info")) # joint evaluation (basically gives the same table)
ls1
```
