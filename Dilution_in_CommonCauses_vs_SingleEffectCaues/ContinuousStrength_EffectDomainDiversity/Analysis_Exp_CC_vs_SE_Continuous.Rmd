...
tdata <- read.delim("Data_Exp_CC_vs_SE_Continuous.txt", header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
...
g <- ggplot(tdata_sub, aes(x=target_cause, y=valueJitter, color = target_cause, fill = target_cause)) +
  ...
  #ggtitle("Subjects' causal srength ratings") +
  scale_y_continuous(limits = c(-0.05, 1.05), breaks=seq(0, 1, 0.1), expand = c(0,0)) +
  scale_x_discrete(labels=c("multiple-effects \n cause", "single-effect \n case")) +
  geom_violinhalf(position=position_dodge(1), alpha = 0.2, color = NA)+
  geom_point(position=position_jitterdodge(dodge.width=0.5), alpha = 0.2) +
  stat_summary(fun.y=mean, colour="black", geom="line", group = 1, size = 1.5, linetype = "solid", alpha = 1)+
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0, size = 1, position = position_dodge(width = 0.5), color = "black") +
...
a1 <- aov_car(Value ~ target_cause*type_effect*target_effect + Error(sID), tdata_sub)
a1
```
Main effect of target cause, as predicted by theory. Also a smaller main effect for type of effect (i.e., effect domains). Figures show that effect is a bit bitter for effects from the same domain. Importantly, there are no interactions. 
...
same_domain %>% cohens_d(Value ~ target_cause, paired = FALSE)
diff_domain %>% cohens_d(Value ~ target_cause, paired = FALSE)
tdata_sub %>% cohens_d(Value ~ target_cause, paired = FALSE)
...
SEd <- sqrt(((n1 + n0)/(n1*n0))+((d^2)/(2*(n1+n0))))
